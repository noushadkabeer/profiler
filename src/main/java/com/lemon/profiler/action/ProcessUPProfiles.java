...
public class ProcessUPProfiles extends ActionSupport {
	...
	public String prepareUPDashboard() {
		...
		String org = ActionContext.getContext().getSession().get(ProfilerConstants.PROPERTY_USER_ORGANIZATION)
				.toString();
		...
		// unprocessedProfiles = uppservice.obtainUnProcessedProfileList(" ", pageNum,
		// pageSize);
		unprocessedProfiles = searchResults.getResults();

		log.debug("Returning the list of size :> " + unprocessedProfiles.size());
		// for(Profile profile: unprocessedProfiles){
		// log.debug(profile.id+ profile.attachments.getId());
		// log.debug(profile.getAttachments().getDownloadURL());
		// }
		// read the json value
		// String jsonContentURL =
		// unprocessedProfiles.get(0).getAttachments().getDownloadURL();
		// download it

		// prepare the new list with auto populated values

		// log.debug("Received cons"+unprocessedProfiles.size());
		return "success";
	}
	...
	public String insertOrUpdate() {
		...
		if (!validationSuccessful()) {
			...
			log.debug("Profile insert.. for " + upProfile.id);
	...
	public String executed() {
		try {
			log.debug("Request received to render >>" + upProfileId);
			...
			if (!"failure".equals(transformedNode)) {
				...
				// >>"+propS.getKeyValue("contentServerURL")+propS.getKeyValue("alfrescocontext")+"/d/a/workspace/SpacesStore/"+transformedNode);
				// URL url = new
				// URL(propS.getKeyValue("contentServerURL")+propS.getKeyValue("alfrescocontext")+"/d/a/workspace/SpacesStore/"+transformedNode);
				...
				// File dir = new File(
				// ServletActionContext.getRequest().getSession().getServletContext().getRealPath("/pdfjs"));
				...
				// log.debug(url+" >> O_o :> "+ file);
				// fileUrl = file.getPath();
				// log.debug(fileUrl);
				// //FileOutputStream file = new FileOutputStream(new
				// File("/pdfjs/temp/001.pdf"));
				// //FileUtils.copyURLToFile(url, file);
				// actionUrl = actionPath+"?file="+savePDF(transformedNode);
				// log.debug(" Returning : "+actionUrl);
				return "success";
			} else {
				return "failure";
			}
	...
	public String readJSON(String uId) {
		...
		JSONParser parser = new JSONParser();
		try {
			String strURL = propS.getKeyValue("contentServerURL") + propS.getKeyValue("alfrescocontext")
					+ "/service/api/node/content/workspace/SpacesStore/" + uId + "?alf_ticket="
					+ new ProfilerUtil().getAlfTicket("admin", "admin");
	...
	public static void main(String args[]) {
		HttpClient client = new HttpClient();
		try {
			String strURL = "http://127.0.0.1:8080/alfresco/service/api/node/content/workspace/SpacesStore/ad34a51e-ab14-41fd-ab89-4bdd0c3cff9f?alf_ticket="
					+ new ProfilerUtil().getAlfTicket("admin", "admin");
			...
			// method.getParams().setParameter("alf_ticket", new
			// ProfilerUtil().getAlfTicket("admin", "admin"));
			int statusCode = client.executeMethod(method);
			log.debug(statusCode);
			if (statusCode != -1) {
				BufferedInputStream bis = new BufferedInputStream(method.getResponseBodyAsStream());
				...
				bis.close();
				bos.close();
				log.debug("C:\\Noushad\\Personal\\Profiler\\src\\main\\webapp\\pdfjs" + name);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main2(String args[]) {
		...
		try {
			// dhttpclient.getCredentialsProvider().setCredentials(new AuthScope(host,
			// AuthScope.ANY_PORT), new UsernamePasswordCredentials(username, password));
			HttpGet dhttpget = new HttpGet(uri);

			URI urid = new URIBuilder(dhttpget.getURI()).addParameter("alf_ticket", ticket).build();
			((HttpRequestBase) dhttpget).setURI(urid);
			log.debug("executing request " + dhttpget.getRequestLine());
			...
			log.debug(dresponse.getStatusLine() + "C:\\Noushad\\Personal\\Profiler\\src\\main\\webapp\\pdfjs" + name);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void mains(String args[]) {
		try {
			String actionPath = "/pdfjs/viewer.jsp";
			URL url = new URL(
					"http://127.0.0.1:8080/alfresco/d/a/workspace/SpacesStore/d26f891f-fc4d-4c24-8015-ff6735484b1c/9826661CV.pdf");
			String myUrl = URLDecoder.decode(url.toString(), "UTF-8");
			String fileUrl = myUrl.replace("\\/", "/").replace(" ", "%20");
			String ext = "pdf";
			File dir = new File("C:\\Noushad\\Personal\\Profiler\\src\\main\\webapp\\pdfjs");
