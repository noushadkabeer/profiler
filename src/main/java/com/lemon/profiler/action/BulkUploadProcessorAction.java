	...
	public String bulkUploadProcessor() throws Exception {
		...
		try {
			if ((filesPath == null || filesPath.equals(""))) {
				...
				String[] cmd = {
					      "py",
					      "C:/Users/emper/PycharmProjects/profileR/venv/One.py", filesPath+"temp\\",
					    };
			...
			//	ru.uploadFiles(file, processed);
				...
				for (File file : listOfFiles) {
					if (file.isFile()) {
						...
						System.out.println("Normal file? "+folder+"\\temp\\"+FilenameUtils.removeExtension(file.getName())+"\\"+file.getName()+".pdf.json");
						jsonFile = new File(folder+"\\temp\\"+FilenameUtils.removeExtension(file.getName())+"\\"+FilenameUtils.removeExtension(file.getName())+".pdf.json");
